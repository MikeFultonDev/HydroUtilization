#!/usr/bin/env python3
"""
Test: Auto-detect daily CSV file in input directory
"""
...
def run_test():
    """
    Test that the script processes daily CSV file with explicit specification.
    Since auto-detection requires only one file, we test explicit file specification instead.
    
    Returns:
        tuple: (passed, message)
    """
    try:
        # Check that input directory exists and has the daily CSV file
        input_dir = Path('input')
        if not input_dir.exists():
            return (False, "input/ directory does not exist")
        
        csv_file = input_dir / 'bchydro.com-daily-consumption.csv'
        ...
        # Check that it processed the daily file
